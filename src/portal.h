...
class Portal {
   ...
   public:
    ...
    // 运行子查询并返回结果
    SelectResult run_and_get_result(std::shared_ptr<PortalStmt> portal,
                                    QlManager* ql, Context* context) {
        switch (portal->tag) {
            case PORTAL_ONE_SELECT: {
                return ql->select_from_and_get_return(
                    std::move(portal->root), std::move(portal->sel_cols),
                    context);
            }
            default: {
                throw InternalError("Unexpected field type");
    ...
    std::unique_ptr<AbstractExecutor> convert_plan_executor(
        ...
        if (auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)) {
            ...
            if (x->tag == T_SeqScan) {
                ...
                // for (const auto& [tab_name, need] : need_rebuild_index) {
                //     if (need) {
                //         std::thread([this, tab_name, context]() {
                //             sm_manager_->rebuild_index(tab_name, context);
                //         }).detach();
                //         need_rebuild_index[tab_name] = false;
                //     }
                // }
