...
auto analyze = std::make_unique<Analyze>(sm_manager.get(), optimizer.get(),
                                         ql_manager.get(), portal.get());
...
void* client_handler(void* sock_fd) {
    ...
    while (true) {
        ...
        Context* context =
            new Context(lock_manager.get(), log_manager.get(), nullptr,
                        data_send, &offset, output_ellipsis);
        ...
        } else if (!output_ellipsis) {
            // 语法解析失败，需要返回给客户端failure信息
            std::fstream outfile;
