...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid,
    ...
    if (context != nullptr)
        context->lock_mgr_->lock_shared_on_record_wait(context->txn_, rid, fd_);
    RmPageHandle rph = fetch_page_handle(rid.page_no);
    char* data = rph.get_slot(rid.slot_no);
    std::unique_ptr<RmRecord> ret =
        std::make_unique<RmRecord>(rph.file_hdr->record_size, data);
    return ret;
}
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if (context != nullptr)
        context->lock_mgr_->lock_exclusive_on_table_wait(context->txn_, fd_);
...
void RmFileHandle::delete_record(const Rid& rid, Context* context) {
    ...
    if (context != nullptr)
        context->lock_mgr_->lock_exclusive_on_record_wait(context->txn_, rid,
                                                          fd_);
    if (rid.page_no >= file_hdr_.num_pages) {
